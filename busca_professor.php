            ...
            <div class="row">
                ...
                <div class="col-md-10" id="conteudo">
                    ...
                    <section> 
                        ...
                        if ($opcao == "id") {
                            $sql_seleciona = "SELECT id_inep, cpf, nome, id, matricula FROM docente WHERE id_inep LIKE '%$chave%'";
                            $resultado = seleciona($sql_seleciona);
                        } else if ($opcao == "nome") {
                            $sql_seleciona = "SELECT id_inep, cpf, nome, id, matricula FROM docente WHERE nome LIKE '%$chave%'";
                            $resultado = seleciona($sql_seleciona);
                        } else if ($opcao == "cpf") {
                            $sql_seleciona = "SELECT id_inep, cpf, nome, id, matricula FROM docente WHERE cpf LIKE '%$chave%'";
                            $resultado = seleciona($sql_seleciona);
                        } else if ($opcao == "matricula") {
                            $sql_seleciona = "SELECT id_inep, cpf, nome, id, matricula FROM docente WHERE matricula LIKE '%$chave%'";
                            $resultado = seleciona($sql_seleciona);
                        } else if ($opcao == "departamento") {
                            $sql_seleciona = "SELECT id_inep, cpf, nome, id, matricula FROM docente WHERE departamento LIKE '%$chave%'";
                            $resultado = seleciona($sql_seleciona);
                        }
                        ?>
